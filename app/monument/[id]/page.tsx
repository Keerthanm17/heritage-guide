...
import { translateMonumentFieldsWithGemini } from "@/lib/translation"
...
const OFFLINE_TRANSLATIONS: Record<string, Record<string, { name: string; description: string; location: string }>> = {
  "qutub-minar": {
    ...
    te: {
      name: "కుతుబ్ మినార్",
      description:
        "కుతుబ్ మినార్ ఢిల్లీలో ఉన్న ఒక ప్రసిద్ధ చారిత్రక స్మారకం. ఇది 73 మీటర్ల ఎత్తైన గోపುరం, ఎరుపు ఇసుకరాయి మరియు పాలరాయితో నిర్మించబడింది. దీని నిర్మాణాన్ని 1193లో కుతುబ్-ఉద్-దీన్ ఐబక్ ప్రారంభించారు।",
  ...
  "chitradurga-fort": {
    ...
    te: {
      name: "చిత్రదుర్గ కోట",
      description:
        "చిత్రదుర్గ కోట కర్ణాటక రాష్ట్రంలోని చిత్రదుర్గ జిల్లాలో ఉన్న ఒక చారిత్రక కోట. ఈ కోట దాని ప్రత్యేకమైన వాస్తుశిల్పం మరియు రక్షణాత్మక నిర్మాణానికి ప్రసిద్ధి చెందింది। దీనిని 'కల్లಿన కోటె' అని కూడా అంటారు।",
...
export default function MonumentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [translationProgress, setTranslationProgress] = useState<{ current: number; total: number; field: string } | null>(null)
  ...
  useEffect(() => {
    const resolveParams = async () => {
      try {
        ...
        setMonumentId(resolvedParams.id)
      } catch (error) {
  ...
  const handleLanguageChange = async (language: string) => {
    ...
    try {
      ...
      if (translation) {
        ...
        // Use Gemini API to translate remaining fields
        const fullyTranslatedMonument = await translateMonumentFieldsWithGemini(
        ...
        // No manual translation available, use Gemini API for all fields
        const fullyTranslatedMonument = await translateMonumentFieldsWithGemini(
          monument as Monument, 
          language,
          (progress) => setTranslationProgress(progress)
          ...
          {isTranslating && (
            <div className="mt-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-blue-800 text-sm mb-2">
                🔄 Translating content to {getLanguageName(currentLanguage)}...
              </p>
              {translationProgress && (
                <div className="space-y-2">
                  <div className="flex justify-between text-xs text-blue-700">
                    <span>Progress: {translationProgress.current} / {translationProgress.total}</span>
                  ...
                  <p className="text-xs text-blue-600">
                    Currently translating: {translationProgress.field}
                  </p>
                </div>
              )}
            </div>
